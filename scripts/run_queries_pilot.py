...
DISCLAIMER = (
    "> **Nota de segurança**: este relatório é apenas *evidência recuperada* do acervo (chunk_id + fonte). "
    "Não é orientação clínica para paciente real.\n"
)

LEXMODE_NOTE = (
    "> **lex_mode**: `STRICT` usa AND (plainto_tsquery). "
    "`RELAX` é fallback determinístico (OR via websearch_to_tsquery) quando STRICT retorna 0 hits.\n"
)
...
def load_queries(seed_path: Path) -> list[str]:
    data = json.loads(seed_path.read_text(encoding="utf-8"))
    if not isinstance(data, list) or not all(isinstance(x, str) for x in data):
        raise ValueError("Seed JSON must be a list[str].")
    return [q.strip() for q in data if q.strip()]


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--seed", default="reports/queries_pilot_seed.json")
    ap.add_argument("--out", default="dataset/out/reports/queries_pilot.md")
    ...
    queries = load_queries(seed_path)
    ...
    lines: list[str] = []
    lines.append("# UpToWes — Queries Pilot Report\n\n")
    ...
    lines.append("> **precision gate (RELAX)**: tenta `QUERY_REDUCED` (anchor + termos raros). Se ainda RELAX, só aceita hits contendo ao menos 1 termo raro.\n")

    with connect() as conn:
        for idx, q in enumerate(queries, start=1):
            reduced_q, terms, must_terms = reduce_query(q)
            ...
            if hits and hits[0].lex_mode == "RELAX" and reduced_q != q:
                hits2 = hybrid_search(
                    conn,
                    reduced_q,
                    top=args.top,
            ...
            if not hits:
                lines.append("- **NO_HITS**: nenhum resultado (ou RELAX filtrado pelo precision gate).\n")
                continue
